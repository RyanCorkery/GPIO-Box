...
void setup() {
  ...
  digitalWrite(manual_swtiches_enable, LOW);                                      // Disable switches 

  digitalWrite(latency_led, HIGH);
...
void menu() {                                       // UI button pressed, execute menu
  ...
  if (digitalRead(menu_run) && mode == 0) {                             // Program Start/Stop button pressed
    ...
    case 1:                                                             // Layer 1 = Mode, Program Speed, Program Number
      ...
      case up:
        ...
        break;
      case down:
        if (index_layer_1_option < 2 && mode == 0) index_layer_1_option++;            // Increment through options. Only increment option when in program mode. No need to change these settings unless we are in program mode
...
void manual_mode() {
  ...
  if (millis() - manual_time >= manual_update) {                    // Check if update time has been reached
    for (int i = 0; i < 10; i++) {                                  // 8 speed signals and 2 analog outputs
      ...
      if (val <= 50) val = 0;  //105                                // 4-20ma -> 1-5V -> 50-255 analog write
      else if (val >= 255) val = 255;  //236
...
void SD_write() {                                               // Write data to SD card
  ...
  if (my_file) {
    ...
    my_file.print('\n');
    my_file.print("00000000000000000000000000000000005000");
    my_file.print('\n');
